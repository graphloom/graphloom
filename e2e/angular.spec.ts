// P5 close-out: the Angular demo (apps/examples-angular) in real browsers.
// Served page is SSR-prerendered output — hydration is exercised on every
// test's goto. Committing gestures carry an undo assertion (P4 convention).
//
...
const nodePosition = (page: Page, id: string): Promise<{ x: number; y: number } | undefined> =>
  page.evaluate((nodeId) => window.angularDemo.editor.graph.getNode(nodeId)?.position, id);
...
test('SSR + hydration: server HTML is real, hydration is warning-free', async ({ page }) => {
  ...
  expect(problems.filter((text) => /ng0|hydration/i.test(text))).toEqual([]);
});
...
test('wheel zooms, space+drag pans; the model is untouched', async ({ page }) => {
  ...
  await expect
    .poll(() => page.evaluate(() => window.angularDemo.host.viewport.viewport.zoom))
    .toBeGreaterThan(1);
  await page.keyboard.down(' ');
  await drag(page, [300, 300], [360, 330]);
  await page.keyboard.up(' ');
  const viewport = await page.evaluate(() => window.angularDemo.host.viewport.viewport);
...
test('Tier-2 overlay stays pixel-locked to the canvas across pan/zoom', async ({ page }) => {
  const overlay = page.locator('.graphloom-overlay-node[data-node-id="gamma"]');
  ...
  for (const viewport of [
    ...
    await page.evaluate((v) => window.angularDemo.host.viewport.setViewport(v), viewport);
    await page.evaluate(() => window.angularDemo.host.renderNow());
    // Core math is the contract: overlay box ≡ worldToScreen(node) at zoom.
    const screen = await page.evaluate(() =>
      window.angularDemo.host.viewport.worldToScreen({ x: 270, y: 340 }),
    );
  ...
  await page.evaluate(() => window.angularDemo.host.viewport.setViewport({ x: 40, y: 10, zoom: 1.5 }));
  await page.evaluate(() => window.angularDemo.host.renderNow());
  await expect(page).toHaveScreenshot('angular-editor-overlay.png');
});
