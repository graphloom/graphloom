...
test('heap after churn — canvas 500x2000', async ({ page }) => {
  ...
  await page.evaluate(() => window.__bench.churn(500));
  const after = await heapUsed();

  const growthPct = ((after - before) / before) * 100;
  metrics['heap'] = { churnCycles: 500, beforeBytes: before, afterBytes: after, growthPct };
  // Sanity only — the real flat-heap gate is bench-report.mjs (and P9-T04).
